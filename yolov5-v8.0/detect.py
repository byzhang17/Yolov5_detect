...
class DetectAPI:
    def __init__(self, weights='weights/yolov5s.pt', source='data/images', data='data/coco128.yaml', imgsz=None, conf_thres=0.55,
                 ...
                 visualize=False, update=False, project='D:/code/yolov5-v8.0/runs/detect', name='myexp', exist_ok=False, line_thickness=3,
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or self.save_crop or self.view_img:  # Add bbox to image
                            ...
                            annotator.box_label(xyxy, label, color=colors(c, True))
                        if self.save_crop:
                            crop, f = save_one_box(xyxy, imc, file=self.save_dir / 'crops' / names[c] / f'{p.stem}.jpg', BGR=True)
                            crops.append((crop,f))
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=ROOT / 'datasets/pointer/images/val', help='file/dir/URL/glob/screen/0(webcam)')
